...
{
    private const string DemoFontPath = "Assets/Fonts/neodgm SDF.asset";
    private const string MarkerPath = "Assets/Scenes/DemoScenesConfigured_v1.marker.txt";
    ...
    private const string DemoDataPath = "Assets/Demo/Data";

    static DemoSceneSetupBuilder()
    {
        EditorApplication.delayCall += BuildOnce;
    }

    [MenuItem("Tools/Wheel of Fortune/3개 데모 씬 구성")]
    ...
    private static void BuildOnce()
    {
        ...
        File.WriteAllText(MarkerPath, "GameStartScene, IGS and MonsterBattleScene demo configured.\n");
        AssetDatabase.ImportAsset(MarkerPath);
        AssetDatabase.SaveAssets();
        Debug.Log("GameStartScene → IGS → MonsterBattleScene 데모 구성을 완료했습니다.");
    }
    ...
    private static void ConfigureGameStartScene()
    {
        WithScene(GameStartPath, scene =>
        {
            ...
            if (canvas != null && GameObject.Find("DEMO_StartTitle") == null)
            {
                GameObject title = CreateTMP("DEMO_StartTitle", canvas.transform, "운명의 수레바퀴", 56);
    ...
    private static void ConfigureBattleScene()
    {
        WithScene(BattlePath, scene =>
        {
            ...
            Transform previous = canvas.transform.Find("DEMO_BattleUI");
            if (previous != null)
            {
            ...
            GameObject root = CreateUI("DEMO_BattleUI", canvas.transform);
    ...
    private static List<RouletteEffectData> CreateRouletteEffects()
    {
        ...
        if (!AssetDatabase.IsValidFolder(DemoDataPath)) AssetDatabase.CreateFolder("Assets/Demo", "Data");
        ...
        {
            string path = $"{DemoDataPath}/{names[i]}.asset";
    ...
    private static GameObject CreateTMP(string name, Transform parent, string value, float fontSize)
    {
        ...
        text.font = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>(DemoFontPath);
        text.text = value;
