...
public class PlayerAttackEffectAnimator : MonoBehaviour
{
    [Tooltip("플레이어의 실제 공격 확정 시점을 전달하는 전투 관리자입니다.")]
    [SerializeField] private BattleManager battleManager;
    [Tooltip("공격 애니메이션 프레임을 표시할 이미지입니다.")]
    [SerializeField] private Image effectImage;
    [Tooltip("PlayerAtk 폴더의 공격 이미지를 파일명 순서대로 연결한 프레임입니다.")]
    [SerializeField] private Sprite[] frames;
    [Tooltip("각 공격 이미지가 표시되는 시간입니다.")]
    [Min(0.01f)] [SerializeField] private float secondsPerFrame = 0.05f;
    [Tooltip("공격 애니메이션이 시작될 때 함께 재생할 효과음입니다.")]
    [SerializeField] private AudioClip effectSound;
    [Tooltip("공격 효과음의 음량입니다.")]
    ...
    {
        if (battleManager != null) battleManager.PlayerAttackExecuted += Play;
        Hide();
    }
